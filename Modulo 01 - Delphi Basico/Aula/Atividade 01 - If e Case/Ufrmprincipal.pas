...
function xNomeTeste(texto: String): Boolean;
  ...
  begin
    if trim(texto) = 'Anderson' then
      ...
      begin
        ShowMessage('Desculpe, ' + trim(texto) + ' está ausente.');
        Result := False
      end;
  end;
  ...
  begin
    if xNomeTeste(edtText.Text) then
      begin
        ...
        0:
          begin
            ShowMessage('Eu, ' + trim(edtText.Text) + ' sou do nível estagiário.')
          end;

        1:
          begin
            ShowMessage('Eu, ' + trim(edtText.Text) + ' sou do nível Júnior.')
          end;

        2:
          begin
            ShowMessage('Eu, ' + trim(edtText.Text) + ' sou do nível Pleno.')
          end;

        3:
          begin
            ShowMessage('Eu, ' + trim(edtText.Text) + ' sou do nível Sênior.')
          end;

        else
          ShowMessage('Por favor, selecione um Nível')
      ...
      if xNomeTeste(edtText.Text) and (rdgNivel.ItemIndex <> -1) then
        begin
          ShowMessage('Eu, ' + trim(edtText.Text) + ' sou do nível ' + rdgNivel.Items[rdgNivel.ItemIndex]);
        end
      else
        begin
          ShowMessage('Por favor, selecione um Nível')
